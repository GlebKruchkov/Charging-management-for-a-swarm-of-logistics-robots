...
class PathBrain(Brain[Map[Cell], Robot[Cell]]):
    ...
    def __init__(self,
        ...
        self._charger_destinations: dict[Robot[Cell], int] = {}
        self._last = -1
        self._last_charger = -1
        ...
        self._generate_to_charger: dict[Robot[Cell], bool] = {}
    ...
    def get_next_action(self, robot: Robot[Cell]) -> Robot.Action:
        ...
        if self._generate_to_charger[robot] and not robot.is_going_to_charger_:
            ...
            self._assign_charger(robot)
            self._clear_path(robot)
            self._generate_charger_path(robot)
        if self._generate_to_input[robot] and not robot.is_going_to_charger_:
            self._generate_to_input[robot] = False
            self._assign_input(robot)
            self._clear_path(robot)
            self._generate_input_path(robot)
        if self._generate_to_output[robot] and not robot.is_going_to_charger_:
            self._generate_to_output[robot] = False
            self._clear_path(robot)
            self._generate_output_path(robot)
        new_state = self._robots_paths[robot][0]

        if (robot.mail is None and new_state.start - self._model.now >= 1
                and not robot.is_going_to_charger_ and robot.get_curr_charge() < robot.get_threshold()):
            self._generate_to_charger[robot] = True
            self._generate_to_output[robot] = False
            self._generate_to_input[robot] = False
            self._current[robot].remove()
            self._current[robot] = self._robots_reserves[robot].popleft()
            return Robot.Action.nothing
    ...
    def _assign_charger(self, robot: Robot[Cell]):
        self._last_charger = (self._last_charger + 1) % len(self._model.map.charge_ids)
        self._charger_destinations[robot] = self._model.map.charge_ids[self._last_charger]

    def _generate_charger_path(self, robot: Robot[Cell]):
        if self.personal_rest:
            res = self.path_adder(
                robot,
                self._model.now,
                self.robot_type.time_to_charge,
                self._model.map.chargers[self._charger_destinations[robot]],
                self.robots_rests[robot]
            )
        else:
            res = self._add_path_for_closest_rest(
    ...
    def _add_path_for_state(self, robot: Robot[Cell], start_time: float, time_between: float,
        ...
        timed_start = TimedVertex(start, start_reserve.next)
        paths = [{timed_start: (
            [PathSpan(timed_start, timed_start, start_reserve.val.reserve_from, start_time - time_between)],
            timed_start)}]
